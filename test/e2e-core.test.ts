...
test('Steps through the Jest+Next.js Counter example', async () => {
  const chrome = await chromium.launch()
  const page = await chrome.newPage()

  await page.goto('http://localhost:8080/?folder=/source/examples/jest-nextjs')

  // Open basic.test.tsx
  await page.locator('.search-label').click()
  await page.locator('.monaco-findInput > .monaco-inputbox > .ibwrapper > .input').clear()
  await page.locator('.monaco-findInput > .monaco-inputbox > .ibwrapper > .input').type('counter.test')
  await page.locator('.highlight').first().click()
