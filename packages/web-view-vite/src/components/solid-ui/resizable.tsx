...
function ResizableHandle<T extends ValidComponent = 'button'>(props: DynamicProps<T, ResizableHandleProps<T>>) {
  ...
  return (
    <ResizablePrimitive.Handle
      class={cn(
        'relative flex w-[2px] shrink-0 items-center justify-center bg-[var(--vscode-panel-border)] after:absolute after:inset-y-0 after:left-1/2 after:w-1 after:-translate-x-1/2 focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring focus-visible:ring-offset-1 data-[orientation=vertical]:h-[2px] data-[orientation=vertical]:w-full data-[orientation=vertical]:after:left-0 data-[orientation=vertical]:after:h-3 data-[orientation=vertical]:after:w-full data-[orientation=vertical]:after:-translate-y-1/2 data-[orientation=vertical]:after:translate-x-0 [&[data-orientation=vertical]>div]:rotate-90 z-70',
        props.class,
      )}
      {...rest}
    >
      <Show when={props.withHandle}>
        <div class="z-10 flex h-4 w-3 items-center justify-center rounded-sm border bg-[var(--vscode-panel-border)]">
          <svg
