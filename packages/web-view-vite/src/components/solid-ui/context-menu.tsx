...
function ContextMenuContent<T extends ValidComponent = 'div'>(props: PolymorphicProps<T, ContextMenuContentProps<T>>) {
  const [local, others] = splitProps(props as ContextMenuContentProps, ['class'])
  return (
    <ContextMenuPrimitive.Portal>
      <ContextMenuPrimitive.Content
        class={cn(
          'z-50 min-w-32 origin-[var(--kb-menu-content-transform-origin)] overflow-hidden rounded-md border bg-popover p-1 text-popover-foreground shadow-md animate-in outline-none',
          local.class,
        )}
        {...others}
      />
...
function ContextMenuItem<T extends ValidComponent = 'div'>(props: PolymorphicProps<T, ContextMenuItemProps<T>>) {
  const [local, others] = splitProps(props as ContextMenuItemProps, ['class'])
  return (
    <ContextMenuPrimitive.Item
      class={cn(
        'relative flex cursor-pointer select-none items-center rounded-sm px-2 py-1 text-sm outline-none focus:bg-(--vscode-menu-selectionBackground) focus:text-accent-foreground data-[disabled]:pointer-events-none data-[disabled]:opacity-50',
        local.class,
      )}
      {...others}
    />
...
function ContextMenuSeparator<T extends ValidComponent = 'hr'>(props: PolymorphicProps<T, ContextMenuSeparatorProps<T>>) {
...
function ContextMenuSubTrigger<T extends ValidComponent = 'div'>(props: PolymorphicProps<T, ContextMenuSubTriggerProps<T>>) {
...
function ContextMenuSubContent<T extends ValidComponent = 'div'>(props: PolymorphicProps<T, ContextMenuSubContentProps<T>>) {
...
function ContextMenuCheckboxItem<T extends ValidComponent = 'div'>(props: PolymorphicProps<T, ContextMenuCheckboxItemProps<T>>) {
...
function ContextMenuGroupLabel<T extends ValidComponent = 'span'>(props: PolymorphicProps<T, ContextMenuGroupLabelProps<T>>) {
...
function ContextMenuRadioItem<T extends ValidComponent = 'div'>(props: PolymorphicProps<T, ContextMenuRadioItemProps<T>>) {
